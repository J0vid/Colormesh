...
This dataframe that is generated will provide data for individual specimens in rows. Original specimen image names will appear in the 1st column. Subsequent columns identify the point sampled and the color channel. For example, column headers will appear as follows: r27, g27, and b27 which indictes that the values appearing in each column are r,g,b values, respectively, associated with point #27. Following the columns of color data, we provide the X & Y coordinates of the center pixel of each sampling circle. For exmple, you will see x27 and y27 which are the x and y pixel coordinate values, respectively, for point #27.   
